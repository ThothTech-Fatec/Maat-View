...
export const cadastrarPergunta = async (req, res) => {
    try {
        ...
        // Buscar o ID da pesquisa pelo título
        const [categoriaPERG] = await pool.query('SELECT id FROM Categoria_Perguntas WHERE categoria = ?', [categoriaPergunta]);
        ...
        await pool.query('INSERT INTO Perguntas ( sobre, formato, cat_id) VALUES (?, ?, ?)', [sobrePergunta, formatoPergunta, categoriaId]);
        ...
        if (options && options.length > 0) {
            for (const option of options) {
                await pool.query('INSERT INTO Opções (per_id, pes_id, texto) VALUES (?, ?, ?)', [perguntaId, pesquisaId, option]);
            }
        }
...
export const buscarPergCat = async (req, res) => {
    try {
