...
export const buscarPerguntasTemporarias = async (req: Request, res: Response) => {
  try {
    ...
    const [avaliacaoResult] = await pool.query<RowDataPacket[]>(`
    ...
    const [perguntasTemporarias] = await pool.query<RowDataPacket[]>(`
      SELECT 
...
export const buscarCategoriaPesquisa = async (req: Request, res: Response) => {
  try {
    const pesquisaId = parseInt(req.params.pesquisaId);

    const [categoriaResult] = await pool.query<RowDataPacket[]>(`
