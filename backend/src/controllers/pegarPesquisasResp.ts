...
interface Pesquisa {
  ...
  titulo: string;
  descricao: string;
  categoria: string;  // Alterei para "categoria" para ilustrar o caso da categoria
...
const getPesquisasRespondidas = async (req: Request, res: Response): Promise<Response> => {
  const userId = parseInt(req.params.userId); // Obtém o ID do usuário
  const dateFilter = req.query.dateFilter as string || 'all'; // Filtro de data (sem filtro será 'all')
